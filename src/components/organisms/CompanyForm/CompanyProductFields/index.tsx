import { Grid } from '@mui/material';
import {
  ControlledDropdownField,
  ControlledTextField,
} from '@src/components/molecules';
import React from 'react';
import { Control } from 'react-hook-form';

interface CompanyProductFieldsProps {
  control: Control<any>;
  sectionPrefix: string;
}

const CompanyProductFields: React.FC<CompanyProductFieldsProps> = ({
  control,
  sectionPrefix,
}) => {
  return (
    <>
      <Grid size={{ xs: 12, md: 4 }}>
        <ControlledDropdownField
          variant="outlined"
          required
          name={`${sectionPrefix}.productDivision`}
          control={control}
          label="Product Division"
          placeholder="Select product division"
          options={[
            { label: 'Hard Line Electric', value: 'HARD_LINE_ELECTRIC' },
            { label: 'Soft Line', value: 'SOFT_LINE' },
            { label: 'Food', value: 'FOOD' },
          ]}
        />
      </Grid>
      <Grid size={{ xs: 12, md: 4 }}>
        <ControlledTextField
          variant="outlined"
          name={`${sectionPrefix}.productType`}
          control={control}
          label="Product Type"
          placeholder="Enter product type"
        />
      </Grid>

      <Grid size={{ xs: 12, md: 4 }}>
        <ControlledDropdownField
          variant="outlined"
          required
          name={`${sectionPrefix}.distributionArea`}
          control={control}
          label="Distribution Area"
          placeholder="Select distribution area"
          options={[
            { label: 'Local', value: 'LOCAL' },
            { label: 'International', value: 'INTERNATIONAL' },
            { label: 'Regional', value: 'REGIONAL' },
          ]}
        />
      </Grid>
    </>
  );
};

export default CompanyProductFields;
